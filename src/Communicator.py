...
class Communicator(asyncore.dispatcher):
    ...
    def readable(self):
        if self.communicator is not None:
            return not self.communicator.closed #如果已经closed则返回true?
        return True
...
#当communicator的buffer非0，才返回true,即才能调用handle_write
    ...
    def handle_write(self):
        sent = self.send(self.communicator._buffer) #发送了sent字节的数据
        self.communicator._buffer = self.communicator._buffer[sent:] #数组向后移sent个字节
    ...
    def __init__(self, remote=None, communicator=None):
        ...
        if remote is not None:
            try:
                ...
                print "%s:: socket connection failed: %s" % (self.__class__.__name__, e)
        ...
        if not self.http.is_headers():
            ...
            if res[1] is not '':
                ...
                if self.http.path is not '':
                    print "Gateway:: HTTP request: " + self.http.path
                self._buffer = res[2]
            else:
                return None
